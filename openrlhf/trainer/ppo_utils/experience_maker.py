...
from .utils import extract_reward_content
...
class RemoteExperienceMaker(ABC):
    ...
    def make_experience(self, samples_list: List[Experience]) -> List[Experience]:
        ...
        if samples_list[0].rewards is not None:
            ...
            # ======== 1. decode -> factorization =========
            # decode sequences
            decoded_sequences_list = [
                self.tokenizer.batch_decode(sequences, skip_special_tokens=True)
                for sequences in sequences_list
            ]  # List[List[str]]: num_micro_batch * micro_batch_size strings

            structured_inputs_list = [
                [extract_reward_content(text) for text in decoded_sequences]
                for decoded_sequences in decoded_sequences_list
            ]  # shape: num_micro_batch * micro_batch_size

            eos_text = self.reward_tokenizer.eos_token or ""
            flat_structured_inputs = [x + " " + eos_text for batch in structured_inputs_list for x in batch]  # flatten to [N]

            ## ========== 2. re-tokenize ========== ##
            input_encodings = self.reward_tokenizer(
                flat_structured_inputs, padding=True, truncation=True, return_tensors="pt"
            )  # batch [N, seq_len]
            input_ids = input_encodings["input_ids"].clone()
            attention_mask = input_encodings["attention_mask"].clone()
            ...
            # reshape to sequences_list
            num_micro_batch = len(sequences_list)
            micro_batch_size = sequences_list[0].shape[0]
            input_ids = input_ids.view(num_micro_batch, micro_batch_size, -1)
            attention_mask = attention_mask.view(num_micro_batch, micro_batch_size, -1)

            # =========== 3. Batch call reward model ============
            r_refs = self.reward_model_group.async_run_method_batch(
                method_name="forward",
                sequences=input_ids,
                attention_mask=attention_mask,
                pad_sequence=[True] * input_ids.shape[0],
            )
            # r_refs: tensor [num_micro_batch, batch_size] 或类似
