...
def create_dataset(test_data, test_data_ref):
    logger.info("==================================================")
    logger.info("Number of samples: %d", len(test_data))
    logger.info("==================================================")
    logger.info("********** Start **********")
    ...
    sentence_to_dpinfo_test = {
        item["raw_sentence"]: item["raw_sentence_llm_dp_info"]
        for item in test_data_ref
    }
    ...
    for item in test_data:
        ...
        # 查找对应的dp信息
        if raw_sentence not in sentence_to_dpinfo_test:
            logger.warning(f"raw_sentence not found in ref_data: {raw_sentence}")
            continue

        sentence_llm_dp_info_4_refinement = sentence_to_dpinfo_test[raw_sentence]
    ...
    with open("dataset/SciERC/test_data/json_files/processed/SciERC-eval-test.json", "w", encoding="utf-8") as f:
        json.dump(rm_data, f, ensure_ascii=False, indent=4)
    logger.info("Dataset for reward model evaluation has been saved to SciERC-eval-test.json.")
    logger.info("********** Done **********")


if __name__ == "__main__":
    with open("dataset/SciERC/test_data/json_files/raw/test_dataset.json", "r", encoding="utf-8") as f:
        test_data = json.load(f)
    with open("dataset/SciERC/test_data/json_files/raw/SciERC-eval_test_sentence_llm_dp.json", "r", encoding="utf-8") as f:
        test_data_ref = json.load(f)
        
    create_dataset(test_data, test_data_ref)
