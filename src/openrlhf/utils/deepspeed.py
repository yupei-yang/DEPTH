...
class DeepspeedStrategy(ABC):
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            if isinstance(model_to_save, PeftModel) and "lm_head" not in output_state_dict:
                try:
                    output_state_dict["base_model.model.lm_head.weight"] = model_to_save.base_model.model.lm_head.weight.data.cpu()
                except Exception as e:
                    print(e)
                
            # 添加详细的错误信息
            assert len(output_state_dict) == len(state_dict), (
                f"mismatch size output_state_dict({len(output_state_dict)}) and state_dict({len(state_dict)}) "
                f"Missing keys: {set(state_dict.keys())-set(output_state_dict.keys())} "
                f"Extra keys: {set(output_state_dict.keys())-set(state_dict.keys())}"
            )
