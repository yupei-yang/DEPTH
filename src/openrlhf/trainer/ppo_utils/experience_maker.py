...
from .utils import extract_reward_content, evaluate_predictions
...
class NaiveExperienceMaker(ABC):
    ...
    def make_experience(
        ...
        correct_rels: Union[str, List[str]],
        **generate_kwargs,
    ) -> Experience:
        self.actor.eval()
        self.critic.eval()
        ...
        decoded_seq = self.tokenizer.batch_decode(sequences, skip_special_tokens=True)

        batch_p, batch_r, batch_f1 = evaluate_predictions(decoded_seq, correct_rels)
        structured_inputs = [extract_reward_content(text) for text in decoded_seq]
        ...
        # rewards
        r = self.reward_model(reward_inputs.input_ids, reward_inputs.attention_mask)
        ...
        info = {
            ...
            "micro_p": torch.full((batch_size,), batch_p, device=sequences.device),
            "micro_r": torch.full((batch_size,), batch_r, device=sequences.device),
            "micro_f1": torch.full((batch_size,), batch_f1, device=sequences.device),
        }
